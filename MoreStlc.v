...
Fixpoint subst (x:id) (s:tm) (t:tm) : tm :=
  ...
  | tm_let x' t1 t2 => tm_let x' (subst x s t1) (if beq_id x x' then t2 else subst x s t2)
  end.
...
Tactic Notation "step_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "ST_Let" | Case_aux c "ST_LetValue"
  ].
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_Lcase : forall T1 T Gamma t1 t2 xh xt t3,
      ...
      has_type Gamma t2 T ->
      xh <> xt ->
      has_type (extend (extend Gamma xt (ty_List T1)) xh T1) t3 T ->
  ...
  | T_If0 : forall Gamma t1 t2 t3 T,
      ...
      has_type Gamma t2 T ->
      has_type Gamma t3 T ->
      has_type Gamma (tm_if0 t1 t2 t3) T
  | T_Let : forall Gamma x t1 t2 T1 T2,
       has_type Gamma t1 T1 ->
       has_type (extend Gamma x T1) t2 T2 ->
       has_type Gamma (tm_let x t1 t2) T2
  .
...
Proof with eauto.
  ...
  Case "T_Lcase".
    ...
    intros x afiH.
    unfold extend.
    remember (beq_id xt x) as e1.
    remember (beq_id xh x) as e2.
...
Proof with eauto.
  ...
  Case "T_Case".
    destruct IHHtyp2 as [T' EH]...
      exists T'.
      rewrite extend_neq in EH...
      now apply not_eq_beq_id_false...

    destruct IHHtyp3 as [T' EH]...
      exists T'.
      rewrite extend_neq in EH...
      now apply not_eq_beq_id_false...
  Case "T_Lcase".
    ...
    rewrite extend_neq in EH.
    rewrite extend_neq in EH...
    apply not_eq_beq_id_false in H7...
    now apply not_eq_beq_id_false in H4...
  Case "T_Let".
    unfold extend in IHHtyp2.
    destruct IHHtyp2 as [ Tx ] ...
    exists Tx.
    now rewrite (not_eq_beq_id_false x0 x H2) in H...
Qed.
...
>> *)
  ...
  - Case "tm_let".
    ...
    + SCase "x<>y".
      ...
      intros z Hafi. unfold extend.
     remember (beq_id y z) as f. destruct f...
     apply beq_id_eq in Heqf. subst.
     rewrite <- Heqe...
Qed.
...
Proof with eauto.
  ...
  - (* tm_abs *)
    rewrite (IHt (extend Gamma i t)
                 ...
                 T3  H10)...
  - (* tm_pair *)
    rewrite (IHt1 Gamma T4 H8 T2 H2).
    rewrite (IHt2 Gamma T5 H10 T3 H4)...
  - (* tm_fst *)
    remember
      (IHt Gamma (ty_prod T0 T3) H1 (ty_prod T1 T5) H5) as Feq.
    inversion Feq; subst...
  - (* tm_snd *)
    remember
